  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="flex justify-end">
        <div className="flex rounded-full border border-warm-border p-0.5 text-sm">
          <button
            onClick={() => setShowPdf(false)}
            className={`rounded-full px-4 py-1 transition-colors ${
              !showPdf
                ? "bg-warm-ink text-[#FAF7F2]"
      ...
      {showPdf ? (
        <iframe
          src="/Max_Li_Resume_Master_HCI-1.pdf"
          className="w-full h-[80vh] rounded-lg border border-warm-border-light"
        />
      ) : (
        <>
          {/* Experience */}
